...
//    [AlixLibService payOrder:orderID AndScheme:kAppSchema seletor:_resultSEL target:[JSenPayEngine sharePayEngine]];
}
...
- (void)wxPayAction:(NSString *)perpayID
{
...
//        NSLog(@"%@",error);
//    }];
    [self prepay:perpayID];
    
}
- (void)prepay:(NSString *)perpayID {
    //https://api.weixin.qq.com/pay/genprepay?access_token=ACCESS_TOKEN
    
    self.prepayid = perpayID;
...
//    return result;
    ...
    //添加key字段
    [contentString appendFormat:@"key=%@", @"A9B7C1D5E3F4G6H2K0L7M9Z1C2V3B6R4"];
    //得到MD5 sign签名
...
{
//    return [CommonUtil sha1:[NSString stringWithFormat:@"%d", arc4random() % 10000]];
